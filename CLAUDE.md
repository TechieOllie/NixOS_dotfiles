...
| `the-entertaining-nios-desktop` | fully wired, eval-clean, **not installed** | btrfs + 16G swap (system disk) | Still running CachyOS: ASRock B550M Pro4, Ryzen 5 5600X, 16G RAM, RX 6600. The only host importing `profiles/gaming.nix`, and it carries the full niri stack (`features.niri = true`). Two disks: the 1T NVMe is the system disk, addressed by `/dev/disk/by-id/` and currently holding the Windows install the bootstrap replaces; a 2T drive is bulk ext4 storage at `/mnt/storage` (`nofail`, symlinked to `~/Storage`), now attached and addressed by its real `by-id` path (Seagate ST2000DM008, serial ZK3055P8) — it still holds an NTFS partition labelled `Extra` that the install destroys. The 500G SATA SSD running the live CachyOS is deliberately undeclared, so the install leaves it bootable as a fallback. `secrets.nix` provisions the password hash plus a real per-host SSH key (`ol@the-entertaining-nios-desktop`, no passphrase) to `~/.ssh/id_ed25519` — a separate identity from `variables.nix`'s bootstrap key, meant for GitHub. Remaining: the install itself. |
