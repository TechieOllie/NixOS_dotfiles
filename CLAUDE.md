...
- `README.md` — human-facing quick start (build/switch/rollback/bootstrap commands, a one-line-per-directory layout table); deliberately does not restate design rationale, only links to `ARCHITECTURE.md`.
- A short `README.md` in each structural directory (`hosts/`, `profiles/`, `modules/`, `modules/system/`, `modules/services/`, `lib/`, `home/`) — a stable "what belongs here and why" orientation, not a changelog of current contents (that's this file's job). Add one when a new structural directory is introduced, and update an existing one only when that directory's *purpose* changes, not when a file is added/removed inside it.
