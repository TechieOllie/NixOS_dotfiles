...
| `the-entertaining-nios-desktop` | fully wired, eval-clean, **not installed** | btrfs + 16G swap (system disk) | Still running CachyOS: ASRock B550M Pro4, Ryzen 5 5600X, 16G RAM, RX 6600. The only host importing `profiles/gaming.nix`, and it carries the full niri stack (`features.niri = true`) and the only `features.moonshine` (remote streaming, tailnet-only). Two disks: the 1T NVMe is the system disk, addressed by `/dev/disk/by-id/` and currently holding the Windows install the bootstrap replaces; a 2T drive is bulk ext4 storage at `/mnt/storage` (`nofail`, symlinked to `~/Storage`) and still carries a `/dev/CHANGEME-storage` placeholder because it isn't physically attached yet. The 500G SATA SSD running the live CachyOS is deliberately undeclared, so the install leaves it bootable as a fallback. `secrets.nix` provisions the password hash plus a real per-host SSH key (`ol@the-entertaining-nios-desktop`, no passphrase) to `~/.ssh/id_ed25519` — a separate identity from `variables.nix`'s bootstrap key, meant for GitHub. Remaining: that storage placeholder, and the install itself. |
...
- **A Moonshine app entry has to name a path that actually resolves for the *system* service.** `/run/current-system/sw/bin/...` works for anything in `environment.systemPackages`, but not for a Home Manager package: this repo leaves `useUserPackages` off, so `home.packages` land in the user's own `~/.nix-profile` and never appear in `/etc/profiles/per-user`. Heroic is referenced by store path via `lib.getExe` for that reason — free, since `useGlobalPkgs` makes it the same derivation `home/heroic.nix` installs. Entries whose binary comes from `features.gaming` are also gated on that flag, so a streaming-but-not-gaming host doesn't advertise apps that can't launch.
