...
let
  ...
  #
  # Deliberately only a window-rule. A `layout { focus-ring { ... } }` block
  # was considered and dropped: home/niri/cfg/layout.kdl already declares a
  # top-level `layout` node, and a second one in an included file is a
  # duplicate-node hazard rather than a merge. Gaps stay in layout.kdl for the
