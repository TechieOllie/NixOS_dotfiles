...
**Per-app window transparency: niri's `opacity` window-rule first, not an app's own rendering/theme internals.** Zen Browser, Nautilus, and Vesktop are all given a translucent, blurred look via the same mechanism in `home/niri/cfg/rules.kdl`, driven by one shared value (`home/transparency.nix`) rather than a per-app guess:

- `opacity <value>` — a compositor-level alpha multiplier applied to the app's entire composited window, independent of whether the app itself has any transparency support at all. This is the mechanism to reach for first: it works uniformly regardless of an app's own rendering internals, at the cost of dimming the whole window — including its own text — equally, since niri has no concept of text vs. background at this level.
- `draw-border-with-background false` — niri's default focus-ring/border renders as a solid backing behind the window, which shows through a translucent surface. Whether to disable just the background fill (this option) or the ring/border entirely (`focus-ring { off } border { off }`) depends on whether the window actually has gap space around it to draw a ring into: a normal tiled window (Nautilus, Vesktop) does, so `draw-border-with-background false` gives a real ring in that space; a window that's also `open-maximized-to-edges` with zero surrounding gap (Zen) doesn't, so the ring has nowhere to render and needs disabling outright instead.

A deeper, per-panel CSS approach (adding alpha to a Discord theme's own background variables, to distinguish background from text more precisely than a blanket compositor multiplier can) was tried for Vesktop specifically and abandoned — see `CLAUDE.md`'s Phase 6 Vesktop section for the full investigation — since it depended on a third-party theme's internal variable structure holding steady release to release, which proved too fragile in practice. Reach for niri's `opacity` first for any future app wanting this look; only go deeper into an app's own theming internals if niri's blanket approach is genuinely insufficient.
