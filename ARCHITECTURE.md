...
**A deliberate exception: `wallpapers/` is read live, not Nix-store-managed.** Every other Home Manager–owned file in this repo is passed to Home Manager as a Nix path (or inline text) and copied into the Nix store as part of the build — the normal, reproducible way Nix handles static content. Wallpapers are the one exception: `home/noctalia.nix` points Noctalia's `wallpaper.directory` at a plain string path, `/home/<user>/.dotfiles/wallpapers`, not a Nix path into this repo. This requires the repo itself to be cloned to `~/.dotfiles` on every host that uses this module (see `docs/wallpapers.md`) — a manual step, and a deliberate exception to "no second source of truth"/"everything flows through the flake." It was chosen anyway because wallpapers are large (megabytes each), mutable, frequently-added binary assets, not code: routing them through the store would mean every rebuild copies the whole directory again (doubling disk usage until GC, since old generations keep their own copy), and adding a single new wallpaper would require a full `nixos-rebuild switch` to take effect. Pointing at a live clone means one copy of the files and no rebuild needed to pick up a new one — judged a better trade for this specific case than for any other Home-Manager-owned file, precisely because wallpapers aren't config that needs to be reproducible from a single Git commit at rebuild time; they're a content library.
